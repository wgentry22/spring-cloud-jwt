...
public class SpringCloudJwtServerConfigImportSelector extends SpringFactoryImportSelector<SpringCloudJwtServerConfiguration>{
	...
	public String[] selectImports(AnnotationMetadata metadata) {
		...
		boolean autoRegister = attributes.getBoolean("autoScan");
		...
		if (autoRegister) {
			...
			try {
				...
				for (Class<?> clazz : jwtComponentClasses)
					if (Arrays.stream(clazz.getAnnotatedInterfaces()).collect(Collectors.toList()).containsAll(Arrays.asList(SpringCloudJwtComponent.class.getAnnotatedSuperclass(), SpringCloudJwtService.class.getAnnotatedSuperclass())))
