...
public class JwtTokenUtil {
	...
	public String generateToken(final Authentication auth) {
		...
		return Jwts.builder().setSubject(auth.getName())
							 .claim(JwtConstants.AUTHORITIES_KEY, auth.getAuthorities().stream().map(GrantedAuthority::getAuthority)
	...
	private Boolean isTokenExpired(String token) {
		final Date expiration = Jwts.parser().setSigningKey(keyProvider.getPublicKey()).parseClaimsJws(token).getBody().getExpiration();
